...
run_test "Network" "Internal_Client2 -> Internal_FW gateway" \
    "sudo docker exec clab-yw-Internal_Client2 ping -c 2 -W 2 192.168.10.1 >/dev/null 2>&1" 8

run_test "Network" "Internal_Client1 <-> Internal_Client2" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 2 -W 2 192.168.10.20 >/dev/null 2>&1" 8
...
run_test "Network" "Proxy_WAF -> Webserver" \
    "sudo docker exec clab-yw-Proxy_WAF ping -c 2 -W 2 10.0.2.30 >/dev/null 2>&1" 8
...
run_test "Network" "Webserver has correct IP (10.0.2.30)" \
    "sudo docker exec clab-yw-Flask_Webserver ip addr show | grep -q '10.0.2.30'" 5
...
run_test "Network" "Internal_FW can reach Internal subnet" \
    "sudo docker exec clab-yw-Internal_FW ping -c 2 -W 2 192.168.10.10 >/dev/null 2>&1" 8

run_test "Network" "Internal_FW can reach DMZ subnet" \
    "sudo docker exec clab-yw-Internal_FW ping -c 2 -W 2 10.0.2.30 >/dev/null 2>&1" 8

run_test "Network" "External_FW can reach Internet" \
    "sudo docker exec clab-yw-External_FW ping -c 2 -W 2 172.168.1.1 >/dev/null 2>&1" 8
...
run_test "Network" "Internal_Client -> Internet (via External_FW)" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 2 -W 2 172.168.2.1 >/dev/null 2>&1" 10
...
run_test "Network" "DMZ -> Internal BLOCKED" \
    "! sudo docker exec clab-yw-Proxy_WAF timeout 3 ping -c 1 -W 2 192.168.10.10 >/dev/null 2>&1" 5

run_test "Network" "Internet -> Internal BLOCKED" \
    "! sudo docker exec clab-yw-Attacker timeout 3 ping -c 1 -W 2 192.168.10.10 >/dev/null 2>&1" 5

run_test "Network" "Attacker -> Internal Network BLOCKED" \
    "! sudo docker exec clab-yw-Attacker timeout 3 ping -c 1 192.168.10.10 >/dev/null 2>&1" 5
...
run_test "Network" "Logstash can reach Elasticsearch" \
    "sudo docker exec clab-yw-logstash timeout 3 bash -c 'echo > /dev/tcp/172.20.20.3/9200' 2>/dev/null" 5

run_test "Network" "SIEM_PC can reach Kibana (port 5601)" \
    "sudo docker exec clab-yw-siem_pc timeout 3 bash -c 'echo > /dev/tcp/172.20.20.15/5601' 2>/dev/null" 5

run_test "Network" "Internal_FW can reach Logstash (port 5044)" \
    "sudo docker exec clab-yw-Internal_FW timeout 3 bash -c 'echo > /dev/tcp/172.20.20.16/5044' 2>/dev/null" 5
...
run_test "Network" "Internal zone has isolated subnet" \
    "sudo docker exec clab-yw-Internal_Switch ip route show | grep -q '192.168.10.0/24'" 5

run_test "Network" "DMZ zone has isolated subnet" \
    "sudo docker exec clab-yw-DMZ_Switch ip route show | grep -q '10.0.2.0/24'" 5
