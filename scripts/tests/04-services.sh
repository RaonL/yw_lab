...
run_test "Services" "Database has required tables" \
    "sudo docker exec clab-yw-Database psql -U admin_user -d admin_db -c '\\dt' 2>/dev/null | grep -q -E 'users|accounts'" 5

run_test "Services" "Database can be queried" \
    "sudo docker exec clab-yw-Database psql -U admin_user -d admin_db -c 'SELECT 1' 2>/dev/null | grep -q '^.*1'" 5
...
run_test "Services" "Logstash connected to Elasticsearch" \
    "sudo docker logs clab-yw-logstash 2>&1 | grep -i 'connected to es\|elasticsearch' | tail -1 | grep -q 'Connected'" 5
...
run_test "Services" "Webserver connects to Database" \
    "sudo docker exec clab-yw-Flask_Webserver timeout 5 bash -c 'echo > /dev/tcp/10.0.2.70/5432' 2>/dev/null" 5

run_test "Services" "Logstash connects to Elasticsearch" \
    "sudo docker exec clab-yw-logstash timeout 5 bash -c 'echo > /dev/tcp/172.20.20.3/9200' 2>/dev/null" 5

run_test "Services" "Kibana connects to Elasticsearch" \
    "sudo docker exec clab-yw-kibana timeout 5 bash -c 'echo > /dev/tcp/172.20.20.3/9200' 2>/dev/null" 5
