...
run_test "SIEM" "Internal_FW Filebeat configured" \
    "sudo docker exec clab-yw-Internal_FW [ -f /etc/filebeat/filebeat.yml ] && cat /etc/filebeat/filebeat.yml | grep -q 'hosts\\|logstash'" 5

run_test "SIEM" "Internal_FW Filebeat logs directory exists" \
    "sudo docker exec clab-yw-Internal_FW [ -d /var/log/filebeat ] && [ -d /var/lib/filebeat ]" 5
...
run_test "SIEM" "Internal_FW firewall log file exists" \
    "sudo docker exec clab-yw-Internal_FW [ -f /var/log/firewall/firewall-events.log ]" 5

run_test "SIEM" "External_FW firewall log file exists" \
    "sudo docker exec clab-yw-External_FW [ -f /var/log/firewall/firewall-events.log ]" 5

run_test "SIEM" "Internal_FW generating firewall events" \
    "sudo docker exec clab-yw-Internal_FW [ -s /var/log/firewall/firewall-events.log ]" 5
...
run_test "SIEM" "Internal_IDS generates eve.json logs" \
    "sudo docker exec clab-yw-Internal_IDS [ -f /var/log/suricata/eve.json ]" 5

run_test "SIEM" "DMZ_IDS generates eve.json logs" \
    "sudo docker exec clab-yw-DMZ_IDS [ -f /var/log/suricata/eve.json ]" 5

run_test "SIEM" "Suricata rules loaded on Internal_IDS" \
    "sudo docker exec clab-yw-Internal_IDS grep -q 'rule' /var/lib/suricata/rules/*.rules 2>/dev/null || [ -d /var/lib/suricata/rules ]" 5
...
run_test "SIEM" "Logstash beats input listening" \
    "sudo docker exec clab-yw-logstash timeout 2 bash -c 'echo > /dev/tcp/127.0.0.1/5044' 2>/dev/null" 5

run_test "SIEM" "Logstash output to Elasticsearch configured" \
    "grep -r 'elasticsearch' /home/turtle/yw_lab/yw_lab/config/logstash/pipeline/ | grep -q '9200'" 5
