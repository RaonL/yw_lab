...
run_test "Firewall" "External_FW: IP Forwarding enabled" \
    "sudo docker exec clab-yw-External_FW cat /proc/sys/net/ipv4/ip_forward | grep -q '1'" 5

run_test "Firewall" "SIEM_FW: IP Forwarding enabled" \
    "sudo docker exec clab-yw-SIEM_FW cat /proc/sys/net/ipv4/ip_forward | grep -q '1'" 5

run_test "Firewall" "Internal_FW: Conntrack enabled" \
    "sudo docker exec clab-yw-Internal_FW cat /proc/sys/net/netfilter/nf_conntrack_max | grep -v '^0$' >/dev/null" 5

run_test "Firewall" "Internal_FW: Default INPUT policy is DROP" \
    "sudo docker exec clab-yw-Internal_FW iptables -L -n | grep -A 1 '^Chain INPUT' | grep -q 'DROP'" 5

run_test "Firewall" "Internal_FW: Default FORWARD policy is DROP" \
    "sudo docker exec clab-yw-Internal_FW iptables -L -n | grep -A 1 '^Chain FORWARD' | grep -q 'DROP'" 5

run_test "Firewall" "External_FW: Default FORWARD policy is DROP" \
    "sudo docker exec clab-yw-External_FW iptables -L -n | grep -A 1 '^Chain FORWARD' | grep -q 'DROP'" 5
...
run_test "Firewall" "External_FW: SNAT rules configured" \
    "sudo docker exec clab-yw-External_FW iptables -t nat -L -n | grep -q 'SNAT'" 5

run_test "Firewall" "Internal_FW has SNAT for Internet traffic" \
    "sudo docker exec clab-yw-Internal_FW iptables -t nat -L -n | grep -q -i 'masquerade\\|snat'" 5
...
run_test "Firewall" "Internal_FW: ESTABLISHED connections allowed" \
    "sudo docker exec clab-yw-Internal_FW iptables -L FORWARD -n | grep -q 'ESTABLISHED'" 5

run_test "Firewall" "Internal_FW: Return traffic allowed" \
    "sudo docker exec clab-yw-Internal_FW iptables -L FORWARD -n | grep -q 'RELATED.*ACCEPT'" 5
...
run_test "Firewall" "Port 8080 (Webserver) rule exists in Internal_FW" \
    "sudo docker exec clab-yw-Internal_FW iptables -L FORWARD -n | grep -q '8080'" 5

run_test "Firewall" "Port 5044 (Logstash) rule exists in related FW" \
    "sudo docker exec clab-yw-SIEM_FW iptables -L -n | grep -q '5044'" 5

run_test "Firewall" "Port 9200 (Elasticsearch) rule exists in SIEM_FW" \
    "sudo docker exec clab-yw-SIEM_FW iptables -L -n | grep -q '9200'" 5
...
run_test "Firewall" "Internal_FW: NFLOG rules configured" \
    "sudo docker exec clab-yw-Internal_FW iptables -L -n | grep -q 'NFLOG'" 5

run_test "Firewall" "External_FW: NFLOG rules configured" \
    "sudo docker exec clab-yw-External_FW iptables -L -n | grep -q 'NFLOG'" 5

run_test "Firewall" "Internal_FW: ulogd logging active" \
    "sudo docker exec clab-yw-Internal_FW ls -l /var/log/firewall/firewall-events.log >/dev/null 2>&1" 5

run_test "Firewall" "External_FW: ulogd logging active" \
    "sudo docker exec clab-yw-External_FW ls -l /var/log/firewall/firewall-events.log >/dev/null 2>&1" 5
...
run_test "Firewall" "SIEM_FW accepts Kibana UI (5601)" \
    "sudo docker exec clab-yw-SIEM_FW iptables -L -n | grep -q '5601'" 5
...
run_test "Firewall" "Internal_FW: Custom LOG_INVALID chain exists" \
    "sudo docker exec clab-yw-Internal_FW iptables -L -n | grep -q 'LOG_INVALID'" 5
