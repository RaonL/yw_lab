...
run_test "Security" "DMZ cannot access Internal network directly" \
    "! sudo docker exec clab-yw-Proxy_WAF timeout 3 ping -c 1 192.168.10.10 >/dev/null 2>&1" 5
...
run_test "Security" "Attacker cannot access Database directly" \
    "! sudo docker exec clab-yw-Attacker timeout 3 ping -c 1 10.0.2.70 >/dev/null 2>&1" 5

run_test "Security" "Internal user cannot access Internet directly (should route through FW)" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 1 -W 3 172.168.2.1 >/dev/null 2>&1" 8
...
run_test "Security" "External user can access Webserver (through NAT)" \
    "sudo docker exec clab-yw-Attacker curl -s -m 5 http://172.168.3.5:8443 >/dev/null 2>&1 || true" 8

run_test "Security" "Database not directly accessible from Internet" \
    "! sudo docker exec clab-yw-Attacker timeout 3 bash -c 'echo > /dev/tcp/10.0.2.70/5432' 2>/dev/null" 5

run_test "Security" "SSH access limited to authorized subnets" \
    "sudo docker exec clab-yw-Internal_FW iptables -L INPUT -n | grep -q 'tcp dpt:22'" 5
...
run_test "Security" "IDS generating event logs (eve.json)" \
    "sudo docker exec clab-yw-Internal_IDS [ -f /var/log/suricata/eve.json ] && [ -s /var/log/suricata/eve.json ]" 5
