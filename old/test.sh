...
run_test "Internal_Client1 running" \
    "sudo docker ps --filter 'name=clab-yw-Internal_Client1' --format '{{.Names}}' | grep -q 'Internal_Client1'" 5

run_test "Internal_Client2 running" \
    "sudo docker ps --filter 'name=clab-yw-Internal_Client2' --format '{{.Names}}' | grep -q 'Internal_Client2'" 5

run_test "Internal_FW running" \
    "sudo docker ps --filter 'name=clab-yw-Internal_FW' --format '{{.Names}}' | grep -q 'Internal_FW'" 5

run_test "External_FW running" \
    "sudo docker ps --filter 'name=clab-yw-External_FW' --format '{{.Names}}' | grep -q 'External_FW'" 5

run_test "SIEM_FW running" \
    "sudo docker ps --filter 'name=clab-yw-SIEM_FW' --format '{{.Names}}' | grep -q 'SIEM_FW'" 5

run_test "Proxy_WAF running" \
    "sudo docker ps --filter 'name=clab-yw-Proxy_WAF' --format '{{.Names}}' | grep -q 'Proxy_WAF'" 5

run_test "Database running" \
    "sudo docker ps --filter 'name=clab-yw-Database' --format '{{.Names}}' | grep -q 'Database'" 5

run_test "DMZ IDS running" \
    "sudo docker ps --filter 'name=clab-yw-DMZ_IDS' --format '{{.Names}}' | grep -q 'clab-yw-DMZ_IDS$'" 5

run_test "Internal IDS running" \
    "sudo docker ps --filter 'name=clab-yw-Internal_IDS' --format '{{.Names}}' | grep -q 'clab-yw-Internal_IDS$'" 5

run_test "Elasticsearch running" \
    "sudo docker ps --filter 'name=clab-yw-elasticsearch' --format '{{.Names}}' | grep -q 'elasticsearch'" 5

run_test "Logstash running" \
    "sudo docker ps --filter 'name=clab-yw-logstash' --format '{{.Names}}' | grep -q 'logstash'" 5

run_test "Kibana running" \
    "sudo docker ps --filter 'name=clab-yw-kibana' --format '{{.Names}}' | grep -q 'kibana'" 5

run_test "Attacker running" \
    "sudo docker ps --filter 'name=clab-yw-Attacker' --format '{{.Names}}' | grep -q 'Attacker'" 5
...
run_test "Internal_Client1 -> Internal_FW" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 2 -W 2 192.168.10.1 >/dev/null 2>&1" 8

run_test "Internal_Client1 -> internet" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 2 -W 2 internet >/dev/null 2>&1" 8

run_test "Internal_Client1 -> Webserver" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 2 -W 2 10.0.2.30 >/dev/null 2>&1" 8

run_test "Internal_Client2 -> Webserver" \
    "sudo docker exec clab-yw-Internal_Client2 ping -c 2 -W 2 10.0.2.30 >/dev/null 2>&1" 8

run_test "Webserver -> Database" \
    "sudo docker exec clab-yw-Flask_Webserver ping -c 2 -W 2 10.0.2.70 >/dev/null 2>&1" 8

run_test "Proxy -> Database" \
    "sudo docker exec clab-yw-Proxy_WAF ping -c 2 -W 2 10.0.2.10 >/dev/null 2>&1" 8
    
run_test "Attacker -> Internet Router" \
    "sudo docker exec clab-yw-Attacker ping -c 2 -W 2 200.168.1.1 >/dev/null 2>&1" 8

run_test "siem_pc -> Kibana (HTTP)" \
    "sudo docker exec clab-yw-siem_pc timeout 5 curl -s http://10.0.3.18:5601/api/status 2>/dev/null | grep -q 'name'" 10

run_test "siem_pc -> Elasticsearch (HTTP)" \
    "sudo docker exec clab-yw-siem_pc timeout 5 curl -s http://10.0.3.14:9200 2>/dev/null | grep -q 'tagline'" 10
...
run_test "Internal Client -> Firewall -> Webserver Port 8080 (ALLOW)" \
    "sudo docker exec clab-yw-Internal_Client1 curl -s -m 5 http://10.0.2.30:8080 2>/dev/null | grep -q 'Login'" 8

run_test "Internet -> Firewall (NAT 8443) -> Webserver Port 8080 (ALLOW)" \
    "sudo docker exec clab-yw-Attacker curl -s -m 5 http://172.168.3.5:8443 2>/dev/null | grep -q 'Login'" 8

run_test "DMZ -> Internal BLOCKED" \
    "!  sudo docker exec clab-yw-Proxy_WAF timeout 3 ping -c 1 -W 2 192.168.10.10 >/dev/null 2>&1" 8

run_test "Internet -> Internal BLOCKED" \
    "! sudo docker exec clab-yw-Attacker timeout 3 ping -c 1 -W 2 192.168.10.10 >/dev/null 2>&1" 8

run_test "Internal -> Internet (ALLOW)" \
    "sudo docker exec clab-yw-Internal_Client1 ping -c 2 -W 3 172.168.2.1 >/dev/null 2>&1" 10

run_test "NAT Configuration (External FW)" \
    "sudo docker exec clab-yw-External_FW iptables -t nat -L -n | grep -q 'DNAT'" 5
...
run_test "Database PostgreSQL responding" \
    "sudo docker exec clab-yw-Database pg_isready -U admin_use >/dev/null 2>&1" 5
...
run_test "Logstash beat port open" \
    "sudo docker exec clab-yw-logstash timeout 3 sh -c 'echo > /dev/tcp/127.0.0.1/5044' 2>/dev/null" 6
...
run_test "Logstash -> Elasticsearch connection" \
    "sudo docker exec clab-yw-logstash timeout 5 sh -c 'curl -s http://10.0.3.26:9200 >/dev/null 2>&1'" 8
...
run_test "WAF blocks SQLi test (403)" \
    "sudo docker exec clab-yw-Proxy_WAF sh -c 'curl -s -o /dev/null -w \"%{http_code}\" \"http://127.0.0.1:8080/?id=1%27%20or%20%271%27=%271\" | grep -q \"^403$\"'" 8

run_test "IP Forwarding enabled (Firewalls)" \
    "sudo docker exec clab-yw-Internal_FW cat /proc/sys/net/ipv4/ip_forward | grep -q '1'" 5

run_test "Default DROP policy (Internal FW)" \
    "sudo docker exec clab-yw-Internal_FW iptables -L -n | grep -E 'Chain (INPUT|FORWARD)' -A 1 | grep -q 'DROP'" 5

run_test "Default DROP policy (External FW)" \
    "sudo docker exec clab-yw-External_FW iptables -L -n | grep -E 'Chain (INPUT|FORWARD)' -A 1 | grep -q 'DROP'" 5

run_test "SIEM_FW restrictive rules" \
    "sudo docker exec clab-yw-SIEM_FW iptables -L FORWARD -n | grep -q 'DROP'" 5
