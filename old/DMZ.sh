...
log_step "1/8" "Stopping all running yw containers..."
RUNNING=$(sudo docker ps -q --filter "name=clab-yw" 2>/dev/null)
if [ -n "$RUNNING" ]; then
...
log_step "3/8" "Removing all yw containers..."
sudo docker ps -a --filter "name=clab-yw" -q 2>/dev/null | xargs -r sudo docker rm -f || true
sudo docker container prune -f || true
log_ok "All containers removed"

log_step "4/8" "Removing containerlab networks..."
sudo docker network ls --filter "name=clab-yw" -q 2>/dev/null | xargs -r sudo docker network rm 2>/dev/null || true
...
sudo docker volume ls --filter "name=clab-yw" -q 2>/dev/null | xargs -r sudo docker volume rm 2>/dev/null || true
...
REMAINING=$(sudo docker ps -a --filter "name=clab-yw" -q 2>/dev/null | wc -l)
...
REMAINING_NET=$(sudo docker network ls --filter "name=clab-yw" -q 2>/dev/null | wc -l)
...
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER SQL Injection Attempt - UNION SELECT"; flow:established,to_server; content:"UNION"; nocase; content:"SELECT"; nocase; classtype:web-application-attack; sid:1000001; rev:2;)
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER SQL Injection Attempt - OR 1=1"; flow:established,to_server; content:"OR"; nocase; content:"1=1"; nocase; classtype:web-application-attack; sid:1000002; rev:1;)
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER SQL Injection Attempt - admin comment"; flow:established,to_server; content:"admin"; nocase; content:"--"; classtype:web-application-attack; sid:1000003; rev:1;)

# XSS Detection
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER XSS Attempt - script tag"; flow:established,to_server; content:"<script"; nocase; classtype:web-application-attack; sid:1000010; rev:1;)
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER XSS Attempt - img onerror"; flow:established,to_server; content:"<img"; nocase; content:"onerror"; nocase; classtype:web-application-attack; sid:1000011; rev:1;)
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER XSS Attempt - svg onload"; flow:established,to_server; content:"<svg"; nocase; content:"onload"; nocase; classtype:web-application-attack; sid:1000012; rev:1;)

# Directory Traversal
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER Directory Traversal - etc/passwd"; flow:established,to_server; content:"etc/passwd"; nocase; classtype:web-application-attack; sid:1000020; rev:1;)
alert http any any -> $HOME_NET $HTTP_PORTS (msg:"ET WEB_SERVER Directory Traversal - .. / pattern"; flow:established,to_server; content:"../"; classtype:web-application-attack; sid:1000021; rev:1;)

# SSH Brute Force
alert tcp any any -> $HOME_NET $SSH_PORTS (msg:"ET SCAN Potential SSH Brute Force"; flow:to_server; flags:S; threshold:type both, track by_src, count 5, seconds 60; classtype:attempted-dos; sid:1000030; rev:1;)

# DoS Detection
alert icmp any any -> $HOME_NET any (msg:"ET DOS ICMP Flood"; threshold:type both, track by_src, count 100, seconds 10; classtype:attempted-dos; sid:1000040; rev:1;)
alert tcp any any -> $HOME_NET $HTTP_PORTS (msg:"ET DOS HTTP GET Flood"; flow:to_server; content:"GET"; http_method; threshold:type both, track by_src, count 100, seconds 10; classtype:attempted-dos; sid:1000041; rev:1;)
alert tcp any any -> $HOME_NET $HTTP_PORTS (msg:"ET DOS HTTP POST Flood"; flow:to_server; content:"POST"; http_method; threshold:type both, track by_src, count 50, seconds 10; classtype:attempted-dos; sid:1000042; rev:1;)
alert tcp any any -> $HOME_NET any (msg:"ET DOS SYN Flood"; flags:S; threshold:type both, track by_dst, count 100, seconds 10; classtype:attempted-dos; sid:1000043; rev:1;)

# Port Scanning
alert tcp any any -> $HOME_NET any (msg:"ET SCAN Potential Port Scan"; flags:S; threshold:type both, track by_src, count 20, seconds 60; classtype:attempted-recon; sid:1000050; rev:1;)
EOF

log_ok "Suricata configuration created."

log_info "Creating separate log directories for IDS instances..."
mkdir -p ./suricata/logs-dmz
mkdir -p ./suricata/logs-internal
chmod -R 777 ./suricata/logs-dmz
chmod -R 777 ./suricata/logs-internal
  ...
  nodes:
    ...
    IDS:
      ...
      binds:
        ...
        - ./suricata/logs-dmz:/var/log/suricata:rw
    ...
    IDS2:
      ...
      binds:
        - ./suricata/suricata.yaml:/etc/suricata/suricata.yaml:ro
        - ./suricata/rules:/var/lib/suricata/rules:ro
        - ./suricata/logs-internal:/var/log/suricata:rw
      cmd: suricata -i eth1 --af-packet
      cap-add:
        - NET_ADMIN
        - NET_RAW
...
until sudo docker exec clab-yw-elasticsearch curl -s http://localhost:9200/_cluster/health | grep -q '"status":"green"'; do
  sudo docker exec clab-yw-elasticsearch curl -s http://localhost:9200/_cluster/health || true
  sleep 2
done
...
sudo docker exec -i clab-yw-Internal_FW bash <<'EOF'
set -e
...
iptables -A FORWARD -i eth1 -o eth2 -d 10.0.2.30 -p icmp --icmp-type echo-request -m limit --limit 10/min -j NFLOG \
  ...
  --nflog-group 0
iptables -A FORWARD -i eth1 -o eth2 -d 10.0.2.30 -p icmp --icmp-type echo-request -j ACCEPT
...
sudo docker exec -i clab-yw-External_FW bash <<'EOF'
set -e
...
sudo docker exec -u 0 -i clab-yw-Proxy_WAF bash <<'EOF'
set -e
...
sudo docker cp filebeat-8.10.0-x86_64.rpm clab-yw-IDS2:/tmp/filebeat.rpm
sudo docker exec -i clab-yw-IDS2 bash <<'EOF'
set -e
...
sudo docker cp filebeat-8.10.0-x86_64.rpm clab-yw-IDS:/tmp/filebeat.rpm
sudo docker exec -i clab-yw-IDS bash <<'EOF'
set -e
...
sudo docker exec -i clab-yw-Internal_Client1 sh <<EOF
...
sudo docker exec -i clab-yw-Internal_Client2 sh <<EOF
apk add --no-cache curl >/dev/null 2>&1 || true
echo '172.168.2.1    internet' >> /etc/hosts
...
sudo docker exec -i clab-yw-Internal_Switch sh <<'EOF'
set -e
...
sudo docker exec -i clab-yw-DMZ_Switch sh <<'EOF'
set -e
apk add --no-cache iproute2 bridge-utils >/dev/null 2>&1 || true
ip link add name br0 type bridge 2>/dev/null || true
ip link set eth1 master br0 2>/dev/null || true
...
sudo docker exec -i clab-yw-Database sh <<'EOF'
set -e
...
sudo docker exec -i --user root clab-yw-Proxy_WAF sh <<'EOF'
set -e
...
sudo docker exec -i --user root clab-yw-Flask_Webserver mkdir -p /app
sudo docker cp ./webserver-details/app.py clab-yw-Flask_Webserver:/app/app.py
sudo docker exec -i --user root clab-yw-Flask_Webserver sh <<'EOF'
set -e
...
sudo docker exec -i clab-yw-router-edge sh <<'EOF'
set -e
# Interfaces
...
sudo docker exec -i clab-yw-Attacker sh <<EOF
set -e
apt update >/dev/null 2>&1 || true
...
sudo docker exec -i clab-yw-router-internet sh <<'EOF'
set -e
# ensure tooling
if command -v apt >/dev/null 2>&1; then
  apt update >/dev/null 2>&1 || true
...
sudo docker exec -i clab-yw-SIEM_FW bash <<'EOF'
set -e
apt-get update -qq 2>&1 | tail -5
apt-get install -y --no-install-recommends \
    iptables \
...
LOGSTASH_PID=$(sudo docker inspect -f '{{.State.Pid}}' clab-yw-logstash)
...
sudo docker exec -i clab-yw-Admin_PC sh <<EOF
set -e
apk add --no-cache curl >/dev/null 2>&1 || true
...
ELASTICSEARCH_PID=$(sudo docker inspect -f '{{.State.Pid}}' clab-yw-elasticsearch)
...
KIBANA_PID=$(sudo docker inspect -f '{{.State.Pid}}' clab-yw-kibana)
